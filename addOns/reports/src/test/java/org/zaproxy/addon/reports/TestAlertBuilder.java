...
/**
 * A builder to simplify alert creation inside tests and provide a fluent API.
 * The defaults will result in a TEST alert, medium risk, medium confidence and
 * some (but not valid) test content
 */
	...
	public Alert build() {
		...
		if (requestHeader != null) {
			try {
				...
				throw new IllegalArgumentException("requestHeader header not valid:\n" + requestHeader + "\n", e);
			}
		}
		...
		if (responseHeader != null) {
			try {
				...
				throw new IllegalArgumentException("responseHeader header not valid:\n" + responseHeader + "\n", e);
			}
		}

		alert.setDetail(description, uriString, param, attack, otherInfo, solution, reference, evidence, cweId, wascId,
				httpMessage);
