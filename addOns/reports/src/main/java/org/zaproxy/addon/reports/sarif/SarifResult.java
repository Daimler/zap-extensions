...
public class SarifResult implements Comparable<SarifResult> {
	...
	public static class SarifResultBuilder {
		...
		public SarifResultBuilder setBinaryContentDetector(SarifBinaryContentDetector binaryContentDetector) {
		...
		public SarifResult build() {
			...
			String messageText = alert.getOtherInfo(); // if possible use other info which has more details
			...
			handleBody(webRequest.body, requestHeader, httpMessage.getRequestBody(), MAX_ALLOWED_REQUEST_BODY,
					attackVector);
			...
			SarifProtocolData requestProtocolData = SarifProtocolData
					.parseProtocolAndVersion(requestHeader.getVersion());
			...
			handleBody(webResponse.body, responseHeader, httpMessage.getResponseBody(), MAX_ALLOWED_RESPONSE_BODY,
					evidenceSnippet);
			...
			SarifProtocolData responseProtocolData = SarifProtocolData
					.parseProtocolAndVersion(responseHeader.getVersion());
			...
			resultLocation.physicalLocation.region.snippet = SarifMessage.builder()
					.setContentAsPlainText(evidenceSnippet).build();
		...
		private void handleBody(SarifBody sarifBody, HttpHeader header, HttpBody body, int maxAllowedChars,
				String snippet) {
			if (binaryContentDetector.isBinaryContent(header)) {
				sarifBody.binary = bigConentShrinker.shrinkTextContent(encodeBodyToBase64(body), 32000, null);
			} else {
