...
public class SarifToolData {
	...
	// @formatter:off
	private static final SarifToolDataProvider OWASP_ZAP = builder().
			setName("OWASP ZAP").
			setTaxonomyVersion("1.0").// we use 1.0 here - should normally not change
			setTaxonomyComprehensive(true).
			setShortDescription("OWASP ZED Attack proxy").
			setOrganization("OWASP").
			build();
	
	private static final SarifToolDataProvider CWE_WITH_4_4_TAXONOMY = builder().
			setName("CWE").
	...
	/**
	 * Because SARIF tool component data often represents duplicated information and
	 * we do NOT want to maintain same data on separate location this class was
	 * introduced. Data for SARIF tool components as well as for taxonomy is
	 * provided by same provider, so can be easily reused.
	 * 
	 * @author albert
	 *
	 */
	public static class SarifToolDataProvider implements SarifToolComponent, SarifTaxonomyDataProvider {
